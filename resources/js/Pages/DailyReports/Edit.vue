<template>
  <AppLayout
    title="Günlük Rapor Düzenle - SPT İnşaat Puantaj Sistemi"
    :full-width="true"
  >
    <!-- Full-width header -->
          ...
          <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center space-x-4 mb-3">
                ...
                <div>
                  <h1 class="text-2xl lg:text-3xl font-bold text-white">Günlük Rapor Düzenle</h1>
                  <p class="text-green-100 text-sm mt-1">Günlük şantiye raporunu güncelleyin</p>
                </div>
              </div>
            </div>
            ...
            <div class="flex-shrink-0">
              <Link
                :href="route('daily-reports.show', dailyReport.id)"
    ...
    <div class="w-full px-4 sm:px-6 lg:px-8 py-6">
      <form @submit.prevent="submitForm" class="space-y-6">
          ...
          <div class="p-6 space-y-6">
            ...
            <div>
              ...
              <div v-if="existingPhotos.length > 0" class="mb-4">
                <p class="text-sm text-gray-600 mb-2">Mevcut Fotoğraflar:</p>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div v-for="(photo, index) in existingPhotos" :key="index" class="relative">
                    <img :src="`/storage/${photo}`" alt="Existing Photo" class="w-full h-32 object-cover rounded-lg" />
                    <button
                      type="button"
              ...
              <div v-if="photoPreviews.length > 0" class="mt-4">
                <p class="text-sm text-gray-600 mb-2">Yeni Fotoğraflar:</p>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="flex items-center justify-end space-x-4">
          ...
          <button
            v-if="dailyReport.approval_status === 'draft' || dailyReport.approval_status === 'rejected'"
            type="submit"
            name="action"
            value="draft"
...
// Form
const form = useForm({
  project_id: props.dailyReport.project_id,
  report_date: props.dailyReport.report_date,
  weather_condition: props.dailyReport.weather_condition,
  temperature: props.dailyReport.temperature,
  weather_notes: props.dailyReport.weather_notes || '',
  total_workers: props.dailyReport.total_workers,
  internal_workers: props.dailyReport.internal_workers,
  subcontractor_workers: props.dailyReport.subcontractor_workers,
  work_summary: props.dailyReport.work_summary,
  completed_works: [],
  ongoing_works: [],
  planned_works: [],
  has_delays: props.dailyReport.has_delays,
  delay_reasons: [],
  has_accidents: props.dailyReport.has_accidents,
  accident_details: [],
  has_material_shortage: props.dailyReport.has_material_shortage,
  material_shortage_details: [],
  visitors: [],
  equipment_usage: props.dailyReport.equipment_usage || [],
  new_photos: [],
  remove_photos: [],
...
const existingPhotos = ref(props.dailyReport.photos || [])
...
const submitForm = () => {
  ...
  // Yeni fotoğraf veya silinecek fotoğraf varsa FormData kullan
  const options = {
    preserveScroll: true,
    forceFormData: form.new_photos.length > 0 || form.remove_photos.length > 0,
    onSuccess: () => {
...
// Lifecycle
onMounted(() => {
  // Convert arrays to text for text areas
  completedWorksText.value = (props.dailyReport.completed_works || []).join('\n')
  ongoingWorksText.value = (props.dailyReport.ongoing_works || []).join('\n')
  plannedWorksText.value = (props.dailyReport.planned_works || []).join('\n')
  delayReasonsText.value = (props.dailyReport.delay_reasons || []).join('\n')
  accidentDetailsText.value = (props.dailyReport.accident_details || []).join('\n')
  materialShortageText.value = (props.dailyReport.material_shortage_details || []).join('\n')
  visitorsText.value = (props.dailyReport.visitors || []).join('\n')
})
</script>
