<template>
  <AppLayout title="Depo Yönetimi" :full-width="true">
    <!-- Full-width header -->
          ...
          <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center space-x-4 mb-3">
                ...
                <div>
                  <h1 class="text-2xl lg:text-3xl font-bold text-white">Depo Yönetimi</h1>
                  <p class="text-cyan-100 text-sm mt-1">Proje depolarını görüntüleyin ve yönetin</p>
                </div>
              </div>
              ...
              <div class="flex flex-wrap items-center gap-4 lg:gap-6">
                <div class="bg-white/10 backdrop-blur-sm rounded-lg px-3 py-2">
                  <span class="text-cyan-100 text-sm">Toplam Depo:</span>
                  <span class="font-semibold text-white ml-1">{{ warehouses?.total || 0 }}</span>
                </div>
            ...
            <div class="flex items-center space-x-2 py-2">
              <ol class="flex items-center space-x-2">
                ...
                <li class="flex items-center">
                  ...
                  <span class="text-white font-medium text-sm">Depo Yönetimi</span>
                </li>
              </ol>
            </div>
          </div>
    ...
    <div class="w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              ...
              <input
                ...
                @input="applyFilters"
                type="text"
                placeholder="Depo adı, lokasyon..."
                class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all"
              />
            </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200/80 overflow-hidden">
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="warehouse in warehouses.data" :key="warehouse.id" class="hover:bg-gray-50 transition-colors">
                <td class="px-6 py-4">
                  <div>
                    <div class="text-sm font-medium text-gray-900">{{ warehouse.name }}</div>
                    <div class="text-sm text-gray-500">{{ warehouse.location || '-' }}</div>
                  </div>
                </td>
                <td class="px-6 py-4">
                  <div class="text-sm text-gray-900">{{ warehouse.project?.name || '-' }}</div>
                </td>
                <td class="px-6 py-4">
                  <div class="text-sm text-gray-900">{{ warehouse.responsible_user?.name || '-' }}</div>
                </td>
              ...
              <tr v-if="!warehouses.data.length">
                <td colspan="5" class="px-6 py-12 text-center">
                  ...
                  <p class="mt-2 text-sm text-gray-500">Henüz depo eklenmemiş.</p>
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <!-- Pagination -->
        <div v-if="warehouses.data.length" class="px-6 py-4 border-t border-gray-200 bg-gray-50">
          <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="text-sm text-gray-700">
              ...
              arası gösteriliyor
              (Toplam: <span class="font-medium">{{ warehouses.total }}</span>)
            </div>
...
const countActive = (isActive) => {
  return props.warehouses.data.filter(w => w.is_active === isActive).length
}
