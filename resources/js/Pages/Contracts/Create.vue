<template>
  <AppLayout title="Yeni Sözleşme" :full-width="true">
    <!-- Header -->
      ...
      <div class="bg-gradient-to-r from-indigo-600 via-purple-700 to-blue-800 border-b border-indigo-900/20 w-full">
        <div class="w-full px-4 sm:px-6 lg:px-8 py-6">
          <div class="flex items-center">
            ...
            <div class="ml-6">
              <h1 class="text-2xl lg:text-3xl font-bold text-white">Yeni Sözleşme</h1>
              <p class="text-indigo-100 text-sm mt-1">Taşeron veya tedarikçi sözleşmesi oluşturun</p>
            </div>
          </div>
        </div>
            ...
            <div class="flex items-center space-x-2 py-2">
              <ol class="flex items-center space-x-2">
                ...
                <li class="flex items-center">
                  ...
                  <span class="text-xs font-medium text-white">Yeni Sözleşme</span>
                </li>
              </ol>
            </div>
          </div>
    ...
    <div class="w-full px-4 sm:px-6 lg:px-8 py-6">
      <form @submit.prevent="submit" class="space-y-6 max-w-4xl">
        ...
        <div class="bg-white shadow-sm rounded-xl border border-gray-200 p-6 space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select v-model="form.project_id" required class="w-full px-3 py-2 border rounded-lg" :class="form.errors.project_id ? 'border-red-500' : 'border-gray-300'">
            ...
            <div>
              ...
              <select v-model="form.subcontractor_id" required class="w-full px-3 py-2 border rounded-lg" :class="form.errors.subcontractor_id ? 'border-red-500' : 'border-gray-300'">
                <option value="">Seçiniz</option>
                <option v-for="sub in subcontractors" :key="sub.id" :value="sub.id">{{ sub.company_name }}</option>
              </select>
              <p v-if="form.errors.subcontractor_id" class="mt-1 text-sm text-red-600">{{ form.errors.subcontractor_id }}</p>
            </div>
        ...
        <div class="flex justify-end space-x-3">
          <Link :href="route('contracts.index')" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">
            İptal
          </Link>
